...
def test_tiny_acted_together_0():
    with open('resources/tiny.pickle', 'rb') as f:
        ...
        actor1 = 1640
        actor2 = 1532
        assert not lab.acted_together(lab.transform_data(tinydb), actor1, actor2)

def test_tiny_acted_together_1():
    with open('resources/tiny.pickle', 'rb') as f:
        tinydb = pickle.load(f)
        actor1 = 2876
        actor2 = 1532
        assert lab.acted_together(lab.transform_data(tinydb), actor1, actor2)
...
# tiny database bacon number test functions
def test_tiny_bacon_number_0():
    n = 0
    expected = { 4724 }
    with open('resources/tiny.pickle', 'rb') as f:
        db_tiny = pickle.load(f)
        first_result = lab.actors_with_bacon_number(lab.transform_data(db_tiny), n)
        assert isinstance(first_result, set)
        assert first_result == expected

        second_result = lab.actors_with_bacon_number(lab.transform_data(db_tiny), n)
...
# tiny database bacon path test function
def test_tiny_bacon_path():
    ...
    with open('resources/tiny.pickle', 'rb') as f:
        ...
        first_result = lab.bacon_path(lab.transform_data(db_tiny), actor_id)
        second_result = lab.bacon_path(lab.transform_data(db_tiny), actor_id)

        check_valid_path(fset_tiny, first_result, 4724, actor_id, len_expected)
        check_valid_path(fset_tiny, second_result, 4724, actor_id, len_expected)
...
def test_tiny_actor_to_actor_path():
    with open('resources/tiny.pickle', 'rb') as f:
        ...
        first_result = lab.actor_to_actor_path(lab.transform_data(tinydb), actor_1, actor_2)
        second_result = lab.actor_to_actor_path(lab.transform_data(tinydb), actor_1, actor_2)
